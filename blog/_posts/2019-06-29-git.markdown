...
## Search the commit history for a string

To find every commit that added or removed a given string (a "pickaxe" search):

```
git log -S"foo" --format='%h %an %ad %s' --date=short
```

This is great for figuring out when something was introduced or deleted, and who did it. The `-S` flag matches commits where the number of occurrences of `foo` changed, and the `--format`/`--date` options print a compact one-line summary (short hash, author, date, subject).
